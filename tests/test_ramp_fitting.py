...
def test_long_integration():
    ...
    buffsize, save_opt, algo, wt, ncores = 512, True, OLS_DEFAULT, "optimal", "none"
...
def test_utils_dq_compress_final():
    ...
    # Run ramp fit on RampData
    buffsize, save_opt, algo, wt, ncores = 512, False, OLS_DEFAULT, "optimal", "none"
    slopes, cube, optional, gls_dummy = ramp_fit_data(
        ramp_data, buffsize, save_opt, rnoise, gain, algo, wt, ncores, dqflags
    )
...
def test_zeroframe():
    ...
    algo, save_opt, ncores, bufsize = OLS_DEFAULT, False, "none", 1024 * 30000
    slopes, cube, ols_opt, gls_opt = ramp_fit_data(
...
def test_multi_more_cores_than_rows():
    ...
    bufsize, algo, save_opt, ncores = 512, OLS_DEFAULT, False, "all"
    slopes, cube, ols_opt, gls_opt = ramp_fit_data(
        ramp, bufsize, save_opt, rnoise, gain, algo, "optimal", ncores, dqflags
    )
...
def test_new_saturation():
    ...
    save_opt, ncores, bufsize, algo = False, "none", 1024 * 30000, OLS_DEFAULT
    slopes, cube, ols_opt, gls_opt = ramp_fit_data(
        ramp, bufsize, save_opt, rnoise, gain, algo, "optimal", ncores, dqflags
    )
